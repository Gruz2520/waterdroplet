...
async def login_for_access_token(user: auth):
    ...
    :param user: login: string, password: string

    gets user's login and password, then authenticates him and gets its user type to check if
    it's correct to log in

    :return: access_token if successful authorization, else HTTPError
    """
...
async def login_for_business(user: auth):
    ...
    separate function for business log in. authenticates user and gets its user type to check if it's
    correct function to log in. if password is default (00000000) then also returns first_enter flag.
...
async def create_user(user: reg_user, user_type: str):
    ...
    """
    :param user: login: str, password: str; optional email: str, full_name: str
    :param user_type: string

    inner function to create users for testing.

    :return: success if account is not occupied
    """
...
async def send_form(name: str, phone: str, message: str):
    ...
    creates and saves form that user sends from site.
