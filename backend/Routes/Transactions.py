...
async def set_verdict(user_id: int, ipu: str, new_number: str):
    """
    :param user_id: id of user that is being checked for his suspiciency
    :param ipu: user's ipu's name
    :param new_number: new ipu's value

    defines if new ipu's value is supicious depending on previous user's ipu values.

    :return: verdict "Подозрительно" или "Не подозрительно"
    """
    ...
    diff = 0
    if len(info) == 1:
        if (abs(int((datetime.datetime.now() - info[0]['date']).days) * 130 - (
                int(new_number) - int(info[0]['new_number'])))) / (int(new_number) - int(info[0]['new_number'])) < 0.3:
    ...
    if int((datetime.datetime.now() - info[-1]['date']).days):
        return "Не подозрительно"
    if (abs(int((datetime.datetime.now() - info[-1]['date']).days) * diff - (
            int(new_number) - int(info[-1]['new_number'])))) / abs(int(new_number) - int(info[-1]['new_number'])) < 0.3:
        return "Не подозрительно"
    else:
        return "Подозрительно"
...
async def insert_info(func_name, username, sheet, headers):
    ...
    :param username: business' name
    :param sheet: workbook sheet
    :param headers: headers for workbook sheet

    inserts information about validations related to business in excel workbook sheet.

    :return: edited workbook file
    """
    PAGE_SIZE = 15
    offset = 0
...
async def add_transaction(login: str, new_number: str, ipu: str):
    ...
    :param new_number: new value for ipu
    :param ipu: user's ipu's name

    Creates row in database containing information about transaction (previous and current number, ipu's name,
                                                                      cost, verdict)

    :return: id of created transaction if successful else HTTP error. Adds first_value flag if value is first for db.
    """
    try:
...
async def change_trans_status(key: Secret_key, trans_id: int, status: int):
    """
    :param key: secret_key that has only frontend server
    :param trans_id:
    :param status: integer defining status: -1 - declined, 0 - created, 1 - pending, 2 - success.
    :return: HTTP status OK if success else HTTP error
    """
    try:
...
async def scan_photo(photo: UploadFile = File(...), key: str = Form()):
    """
    :param photo: File of ipu's photo
    ...
    creates transaction for user that was defined by QR on photo.

    :return:  transaction info if success else HTTP error with details
    """
...
async def get_transactions_logs(token: Token, page_id: int, first_date: Optional[str] = None,
    ...
    """
    :param token: Token: access_token: string
    :param page_id: int
    :param first_date: optional, string
    :param second_date: optional, string
    :param search: optional, string
    :return: page of transactions logs defined by filters and search function
    """
    try:
...
async def get_suspicious_transactions_logs(token: Token, page_id: int, first_date: Optional[str] = None,
    ...
    separate function for returning suspicious-only logs
    ...
    try:
        ...
        if user_type == "business":
            page_id -= 1
            info, amount = await SQLDatabase.get_sus_transactions_logs(username, page_id, first_date, second_date,
                                                                       search)
            return JSONResponse({'data': info, 'amount': amount['total_rows']})
        else:
            raise HTTPException(status_code=400, detail="bad user_type")
...
async def save_file(token: Token):
    ...
    :return: xlsx file that contaings logs for business defined by its token
    """
    try:
        username, user_type = unpack_token(token.access_token)
        if user_type == "business":
